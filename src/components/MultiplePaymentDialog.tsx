  ...
  useEffect(() => {
    if (open && orderIds.length > 0) {
      ...
      const debt = selected.reduce((sum, order) => {
        ...
        const orderPaid = Number(order.totalPaidAmount || order.totalPaidAmount || order.paidAmount || order.initialPayment) || 0;
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {selectedOrders.map((order) => {
                          ...
                          const paid = Number(order.totalPaidAmount || order.totalPaidAmount || order.paidAmount || order.initialPayment) || 0;
                          const debt = Math.max(0, total - paid);
                          return (
                            <TableRow key={order.id}>
                              <TableCell className="font-medium text-center">{order.code}</TableCell>
